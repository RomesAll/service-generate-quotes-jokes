...
from app.services import UsersService, UsersSchemaPOST, UsersSchemaPUT
...
@router.get("/")
def get_all_users(request: Request, session: session_depends, payload: validate_active_user_depends):
    result = UsersService(session=session, client=request.client.host).select_all_users()
    return {'users': result}

@router.get("/{user_id}")
def get_users_by_id(request: Request, user_id: uuid.UUID, session: session_depends, payload: validate_active_user_depends):
    result = UsersService(session=session, client=request.client.host).select_users_by_id(user_id)
    return {'user': result}

@router.post("/")
def create_users(request: Request, new_object: UsersSchemaPOST,
